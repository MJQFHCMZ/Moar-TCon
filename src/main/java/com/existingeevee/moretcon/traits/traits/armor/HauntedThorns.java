...
public class HauntedThorns extends AbstractArmorTrait {

	public HauntedThorns() {
		super(MiscUtils.createNonConflictiveName("haunted"), TextFormatting.WHITE);
	}

	@Override
	public float onDamaged(ItemStack armor, EntityPlayer player, DamageSource source, float damage, float newDamage, LivingDamageEvent evt) {
		if (source.getTrueSource() instanceof EntityLivingBase && random.nextInt(8) == 0)
			CustomFireHelper.setAblaze((EntityLivingBase) source.getTrueSource(), CustomFireEffect.SPIRIT_FIRE, 120);
		return newDamage;
	}
}